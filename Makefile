...
ifeq ($(OS), Windows_NT)
	...
	BIN_NAME = index.html
	LINKER_FLAGS += --embed-file $(ROMFILE) --shell-file $(WEB_SHELL) -s EXPORTED_FUNCTIONS='["_LoadRomFile", "_main", "_SetButtons", "_PauseEmulation", "_ResumeEmulation", "_takeScreenShot", "_SetPaddleMode","_SetJoystickPaddleMode","_SetJoystickPaddleButton","_SetJoystickHeldButtons", "_SetPaddleTouchMode", "_UpdatePaddleFromMouseJS"]' -s EXPORTED_RUNTIME_METHODS='["ccall","cwrap"]' -lidbfs.js
